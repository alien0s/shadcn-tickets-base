...
function SchoolsGridComponent({ schools, isLoading = false, error }: SchoolsGridProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div className="grid min-h-0 flex-1 auto-rows-max content-start items-start grid-cols-1 gap-3 overflow-auto pb-2 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-5">
      {schools.map((school, index) => {
        ...
        return (
          <article key={school.id} className="min-h-[184px] self-start overflow-hidden rounded-xl border border-border bg-card">
